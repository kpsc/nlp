...
def beam_search(sess, model, vocab, source, source_len, source_extend, oov_size):
    beam_size = model.beam_size
    source = np.tile(source, [beam_size, 1])
    source_extend = np.tile(source_extend, [beam_size, 1])
    ...
    while step < model.max_decode_step and len(results) < beam_size:
        ...
        last_tokens = [t if t < vocab.size else vocab.unk for t in last_tokens]
        states = [h.state for h in hyps]

        topk_idx, topk_log_probs, new_states = model.decode_onestep(
            sess, last_tokens, states, encoder_outputs, source_len, source_extend, oov_size)
