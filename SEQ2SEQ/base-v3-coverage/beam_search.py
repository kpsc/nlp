...
def beam_search(sess, model, vocab, source, source_len):
    ...
    coverage = np.zeros([source.shape[1]])
    hyps = [Hypothesis(tokens=[vocab.start], log_probs=[0.0], state=dec_inp_state, coverage=coverage)
    ...
    while step < model.max_decode_step and len(results) < beam_size:
        ...
        pre_coverage = [h.coverage for h in hyps]

        topk_idx, topk_log_probs, new_states, new_coverage = model.decode_onestep(
            sess, last_tokens, states, encoder_outputs, source_len, pre_coverage)

        all_hyps = []
        num_hyps = 1 if step == 0 else len(hyps)
        for i in range(num_hyps):
            hyps_, new_state, new_cov = hyps[i], new_states[i], new_coverage[i]
