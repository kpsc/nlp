...
class Seq2Seq(object):
    ...
    def add_seq2seq(self):
        ...
        outputs, self.dec_out_state = self.decoder.decoder(self.decoder_embedding,
                                                           self.dec_inp_state,
                                                           self.encoder_outputs,
                                                           self.source_len,
                                                           decode=(self.mode != 'train'))
        ...
        with tf.variable_scope('output'):
            ...
            if self.mode == 'train':
                ...
                self.loss = tf.contrib.seq2seq.sequence_loss(self.prob, self.target, mask)  # default: softmax(prob)
            else:
    ...
    def decode_onestep(self, sess, last_tokens, dec_pre_state, encoder_outputs, source_len):
        '''
        Args:
    ...
    def train_step(self, sess, source, source_len, target, target_len, decoder_input, optim=True):
        feed_dict = {
            self.source: source,
            self.source_len: source_len,
            self.target: target,
